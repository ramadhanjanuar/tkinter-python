...
class ProgramPembayaran(object):
    """docstring for Prog."""
    ...
    def aturKomponen(self):
        #mengatur frame sebagai kontainer utama aturKomponen
        #button, label, dan entry, dengan batas tepi "10"
        mainFrame = Frame(self.parent, bd=10)
        mainFrame.pack(fill=BOTH, expand=YES)

        #pasang komponen
        Label(mainFrame, text='Total Pembelian').grid(row=0, column=0, sticky=W, padx=5, pady=5)
        self.entPembelian = Entry(mainFrame)
        self.entPembelian.grid(row=0, column=1, padx=5, pady=5)
        self.btnHitung = Button(mainFrame, text="Hitung", command=self.onHitung)
        self.btnHitung.grid(row=0, column=2, padx=10, pady=10)

        Label(mainFrame, text='Pajak (10%)').grid(row=1, column=0, sticky=W, padx=5, pady=5)
        self.entPajak = Entry(mainFrame)
        self.entPajak.grid(row=1, column=1, padx=5, pady=5)

        Label(mainFrame, text='Total Pembayaran').grid(row=2, column=0, sticky=W, padx=5, pady=5)
